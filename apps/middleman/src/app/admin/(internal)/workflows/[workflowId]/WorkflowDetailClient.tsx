...
export function WorkflowDetailClient({
  ...
  const query = useQuery({
    ...
    queryFn: async () => {
      ...
      if (!result.success || !result.data) throw new Error(result.error ?? 'Failed to load workflow detail')
      return result.data
    },
  ...
  const downloadHistory = async () => {
    ...
    if (!result.success || !result.data) {
      setDownloadError(result.error ?? 'Failed to export history')
      return
    }
  ...
  const terminate = async () => {
    ...
    try {
      ...
      setTermError(err instanceof Error ? err.message : 'Failed to terminate workflow')
    } finally {
