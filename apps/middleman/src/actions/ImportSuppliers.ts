 ...
 * @param providerIdentity - The identity of the provider being imported from
 * @param providerId - The database ID of the provider
 * @param nonce - The nonce received from the provider
...
export async function UpdateImportAttemptStatus(
  ...
  if (!attempt) {
    throw new Error(`Import attempt ${attemptId} not found`)
  }

  if (attempt.userIdentity !== userIdentity) {
    throw new Error(`Import attempt ${attemptId} not owned by user ${userIdentity}`)
  }
 ...
 * @param suppliers - Array of imported supplier data
 ...
 * Called when user aborts and retries.
 *
 ...
 * Used to exclude them when requesting new suppliers from the provider.
 *
 * @param ownerAddress - The owner address
 * @param providerIdentity - The provider identity
 * @returns Array of already-imported node addresses
 */
