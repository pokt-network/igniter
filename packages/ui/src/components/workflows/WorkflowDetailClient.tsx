...
import type { WorkflowsActions, WorkflowDetailFeedback } from './types'
...
export function WorkflowDetailClient({
  ...
  feedback: WorkflowDetailFeedback
}) {
  ...
  const query = useQuery({
    ...
    queryFn: async () => {
      const result = await actions.GetWorkflowDetail(workflowId, runId)
  ...
  const downloadHistory = async () => {
    if (feedback.mode === 'inline') setDownloadError(null)
    const result = await actions.GetWorkflowHistoryJson(workflowId, runId)
    if (!result.success) {
      const message = result.error.message
      if (feedback.mode === 'toast') feedback.onDownloadError(message)
      else setDownloadError(message)
      return
    }
  ...
  const terminate = async () => {
    ...
    try {
      const result = await actions.TerminateWorkflow(detail.workflowId, detail.runId)
      if (!result.success) throw new Error(result.error.message)
      await query.refetch()
      if (feedback.mode === 'inline') setConfirmTerminate(false)
    } catch (err) {
      const message = err instanceof Error ? err.message : 'Failed to terminate workflow'
      if (feedback.mode === 'toast') feedback.onTerminateError(message)
      else setTermError(message)
    } finally {
      setIsTerminating(false)
      if (feedback.mode === 'toast') setConfirmTerminate(false)
    }
  }
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="space-y-3">
        ...
        {feedback.mode === 'inline' && downloadError && (
      ...
      <Dialog open={confirmTerminate} onOpenChange={(open) => !open && setConfirmTerminate(false)}>
